            ...
            while inMenu do
                ...
                RageUI.IsVisible(RMenu:Get("wipe",'wipe_playerlist'), true, true, true, function()
                    for k,v in pairs(GetActivePlayers()) do 
                        RageUI.ButtonWithStyle(GetPlayerServerId(v).." - "..GetPlayerName(v), nil, { RightLabel = ">> Wipe that player" }, true, function(Hovered, Active, Selected)
                            if Selected then
                ...
                RageUI.IsVisible(RMenu:Get("wipe",'wipe_confirm'), true, true, true, function()
                    RageUI.ButtonWithStyle("~g~Yes", nil, {}, true, function(Hovered, Active, Selected)
                        if Selected then
                            RageUI.CloseAll()
                            inMenu = false
                    ...
                    RageUI.ButtonWithStyle("~r~No", nil, {}, true, function(Hovered, Active, Selected)
                        if Selected then
                            RageUI.CloseAll()
                            inMenu = false
                            SelectedPlayer = nil
